class Home extends egret.Sprite {
    ...
    private createViews(): void {
        ...
        var startBmp: egret.Bitmap = Utils.createBitmap("start-btn_png");
        ...
        var ruleBmp: egret.Bitmap = Utils.createBitmap("rule-btn_png");
    ...
    private showRuleBtn() {
        egret.Tween.get(this.ruleBtn).to({ alpha: 1, y: this.ruleBtn.y - 20 }, 600).call(() => this.showStartBtn());
    }
    ...
    private createCar(container: egret.DisplayObjectContainer) {
        ...
        .call(() => {
            ...
            .to({ x: 180, y: 940, scaleX: 1, scaleY: 1 }, 1300).call(() => this.showRuleBtn());
        });
    }

    private createCloud(container: egret.DisplayObjectContainer) {
    ...
    private move(cloud: egret.Bitmap) {
        var duration = Utils.random(1500, 2000);
        var direction = Utils.random(0, 1);
        var distance = Utils.random(15, 20);
        egret.Tween.get(cloud)
        .to({ x: direction ? cloud.x + distance : cloud.x - distance }, duration).call(() => {
            egret.Tween.get(cloud).to({ x: direction ? cloud.x - distance : cloud.x + distance }, duration).call(() => {
                this.move(cloud);
            });
        });
    }
